...
void cleanup_semaphores(){
    ...
    if(sem_destroy(&semaphore_queue) != 0)
        perror("An error occurred while destroying 'queue' semaphore");

    if(sem_destroy(&semaphore_filled) != 0)
        perror("An error occurred while destroying 'filled' semaphore");

    if(sem_destroy(&semaphore_empty) != 0)
        perror("An error occurred while destroying 'empty' semaphore");

}

void setup_semaphores(){

    if(sem_init(&semaphore_queue, 0, 1) != 0){
        perror("An error occurred while initializing 'queue' semaphore");
        exit(1);
    }

    if(sem_init(&semaphore_filled, 0, 0) != 0){
        perror("An error occurred while initializing 'filled' semaphore");
        exit(1);
    }

    if(sem_init(&semaphore_empty, 0, queue_size) != 0){
        perror("An error occurred while initializing 'empty' semaphore");
        exit(1);
    }
